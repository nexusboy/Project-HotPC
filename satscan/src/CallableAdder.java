...
public class CallableAdder implements Callable<Circles> {
       ...
       int end_point ; 
       double max_llh ; 
       CallableAdder(int i  ,double[][] finalGrid , ArrayList<Coordinates> centres, List<Coordinates> address)
       {	 this.num_i = i ; 
    	   ...
    	   for (int j = 0; j < centres.size() ; j++) {	// For All Other centers in the list 
				if(num_i != j) {	// If they are different centers
					...
					// System.out.println(numOfPoints);
					  double current_llh = loglikelihood(numOfPoints, address.size(), 3.14*finalGrid[num_i][j]*finalGrid[num_i][j], 2500);	    			   
